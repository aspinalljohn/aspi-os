...
The second SessionEnd hook writes a short AI-authored daily log of what the session did to `06 me/session logs/YYYY-MM-DD.md`. Watch for two gotchas: **recursion** (a session-log write can itself trigger another SessionEnd — guard against it) and **inner-settings** (a headless `claude -p` call inherits the same hook, so it tries to log its own run — give the inner call a stripped settings file).

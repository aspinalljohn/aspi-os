...
2. **Scan the vault for related notes.** Grep filenames and content across `01 reading/`, `02 venture-one/`, `03 venture-two/`, `04 venture-three/`, `05 projects/`, `06 me/`, `07 potential skills/` for keywords from the input (key entities, frameworks, people, products). Identify 3–8 candidate notes for cross-linking. Don't search the entire vault — be targeted.

3. **Summarize the content.** Structured markdown, starting with YAML frontmatter:
   - `tags:` — 1-4 from the closed list in [[tag-vocabulary]] (`09 resources/tag-vocabulary.md`), only where the note is substantially on-theme. Closed list only — never invent tags.
   ...
   - **Related:** `[[Internal Links]]` to existing vault notes you identified in step 2 — **minimum 2 links, and one must be the domain hub** (`[[venture-one hub]]`, `[[venture-two hub]]`, `[[venture-three hub]]`, `[[me hub]]`, `[[reading hub]]`, `[[projects hub]]`, `[[resources hub]]`, `[[vault operations hub]]`). If you can't find 2 real connections, route to `00 inbox/` instead — a note linked to nothing doesn't belong in the library yet.
...
- **Digested information / article / thread / research / reference** → `01 reading/<Topic>/`
...
- **Standalone project work (not one of the three ventures)** → `05 projects/<project>/`
- **Personal reflection / journal / health / peptides / linkedin draft** → `06 me/<subfolder>/`
- **Portable prompt blueprint detected** → suggest running `/skillify` (saves to `07 potential skills/`)
